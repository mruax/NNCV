...
def plot_deep_results(y_raw, y_norm, norm_params, deep_nets, deep_labels,
                      n_in, n_out, n_cross, train_idx):
    """Графики для эксперимента с двумя скрытыми слоями."""
    fig = plt.figure(figsize=(20, 14))
    colors = ['red', 'blue', 'green', 'orange', 'purple']

    # 1. Сходимость E_train + E_test
    ax1 = fig.add_subplot(2, 2, 1)
    for idx, (net, label) in enumerate(zip(deep_nets, deep_labels)):
        c = colors[idx % len(colors)]
        ax1.plot(net.history_E_train, '-', color=c, linewidth=1, label=f'{label} train')
        if net.history_E_test:
            ax1.plot(net.history_E_test, '--', color=c, linewidth=1, alpha=0.7, label=f'{label} test')
    ax1.set_xlabel('Аудит')
    ax1.set_ylabel('E')
    ax1.set_title('Сходимость: два скрытых слоя', fontweight='bold')
    ax1.set_yscale('log')
    ax1.legend(fontsize=7)
    ax1.grid(True, alpha=0.3)

    # 2. Воспроизведение на train
    ax2 = fig.add_subplot(2, 2, 2)
    ax2.plot(y_raw[:train_idx], 'k-', linewidth=0.8, alpha=0.3, label='Исходные')
    for idx, (net, label) in enumerate(zip(deep_nets, deep_labels)):
        c = colors[idx % len(colors)]
        r = reconstruct_series(net, y_norm, train_idx, n_in, n_out, n_cross, norm_params)
        rmse_i = np.sqrt(np.nanmean((r - y_raw[:train_idx])**2))
        ax2.plot(r, '-', color=c, linewidth=1, alpha=0.7, label=f'{label} (RMSE={rmse_i:.2f})')
    ax2.set_title('Воспроизведение: два скрытых слоя', fontweight='bold')
    ax2.set_ylabel('Цена, EUR')
    ...
    x_hist = np.arange(len(y_raw))
    ax3.plot(x_hist, y_raw, 'k-', linewidth=0.8, alpha=0.5, label='История')
    forecast_len = int(len(y_raw) * 0.15)
    for idx, (net, label) in enumerate(zip(deep_nets, deep_labels)):
        c = colors[idx % len(colors)]
        fore_norm = forecast_future(net, y_norm, forecast_len, n_in, n_out, n_cross)
        fore = denormalize(fore_norm, norm_params)
        x_fore = np.arange(len(y_raw), len(y_raw) + len(fore))
        ax3.plot(x_fore, fore, '--', color=c, linewidth=1.5, label=f'{label}')
    ax3.axvline(x=len(y_raw), color='orange', linestyle=':', linewidth=2)
    ax3.set_title('Прогноз: два скрытых слоя', fontweight='bold')
    ax3.set_ylabel('Цена, EUR')
    ax3.legend(fontsize=7)
    ax3.grid(True, alpha=0.3)

    # 4. Ретроспектива лучшей deep-сети
    ax4 = fig.add_subplot(2, 2, 4)
    best_deep = min(zip(deep_nets, deep_labels),
                    key=lambda x: min(x[0].history_E_test) if x[0].history_E_test else float('inf'))
    best_net_d, best_label_d = best_deep
    test_len = len(y_raw) - train_idx
    retro = forecast_future(best_net_d, y_norm[:train_idx], test_len, n_in, n_out, n_cross)
    retro_denorm = denormalize(retro, norm_params)
    x_test = np.arange(train_idx, len(y_raw))
    ax4.plot(np.arange(train_idx), y_raw[:train_idx], 'k-', linewidth=0.6, alpha=0.3, label='Train')
    ax4.plot(x_test, y_raw[train_idx:], 'go-', markersize=4, linewidth=1.5, label='Test (реальные)')
    ax4.plot(x_test[:len(retro_denorm)], retro_denorm[:test_len], 'r--o', markersize=3,
             linewidth=1.5, label=f'Прогноз ({best_label_d})')
    ax4.axvline(x=train_idx, color='purple', linestyle=':', linewidth=2)
    rmse_retro = np.sqrt(np.mean((retro_denorm[:test_len] - y_raw[train_idx:])**2))
    ax4.set_title(f'Ретроспектива: лучшая сеть (RMSE={rmse_retro:.2f})', fontweight='bold')
    ax4.set_ylabel('Цена, EUR')
    ax4.legend(fontsize=8)
    ax4.grid(True, alpha=0.3)

    plt.suptitle('Эксперимент: два скрытых слоя', fontsize=15, fontweight='bold')
    plt.tight_layout()
    return fig
...
def main():
    ...
    for layers in deep_configs:
        ...
        deep_nets.append(net_tmp)
        deep_labels.append(label)
    ...
    # 4. Два скрытых слоя
    fig4 = plot_deep_results(y_raw, y_norm, norm_params, deep_nets, deep_labels,
                             N_IN, N_OUT, N_CROSS, split_idx)
    fig4.savefig('plots/deep_layers.png', dpi=150, bbox_inches='tight')
    print("  Сохранено: plots/deep_layers.png")
    plt.close(fig4)
