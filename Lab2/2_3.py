"""
ЛАБОРАТОРНАЯ РАБОТА: АППРОКСИМАЦИЯ ВРЕМЕННОГО РЯДА АКЦИЙ HEINEKEN
Модель из 5 гармоник (ОПТИМИЗИРОВАННАЯ ВЕРСИЯ)
Метод 1: Градиентный спуск для E (МНК) - с адаптивным шагом
Метод 2: Градиентный спуск для ε (МОП) - с нормализацией градиента
"""
...
class HarmonicModel:
    ...
    def initialize_params_greedy(self, t, y):
        ...
        """
        print("  [Init] Запуск умной инициализации параметров...")
        B_init = np.mean(y)
        residual = y - B_init
...
# --- ФУНКЦИИ ИЗ 2_1.py (Визуализация и загрузка) ---
...
def plot_results(t, y, model1, model2, t_test=None, y_test=None,
    ...
    """
    Визуализация результатов (Копия логики 2_1.py)
    """
