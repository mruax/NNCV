...
def sobel_magnitude(image):
    """
    Модуль градиента Собеля: sqrt(Gx² + Gy²).
    Комбинирует производные по x и y.
    """
    gx = convolve2d_fast(image, kernel_sobel_x())
    gy = convolve2d_fast(image, kernel_sobel_y())
...
def detect_objects(image_rgb, blur_sigma=1.5, blur_size=5,
                   edge_method='sobel', threshold=0.15,
...
def plot_kernels():
    ...
    kernels = {
        ...
        'Собель X': kernel_sobel_x(),
        'Собель Y': kernel_sobel_y(),
    }
...
def plot_filter_effects(image_rgb):
    ...
    filters = {
        ...
        'Собель X': np.abs(convolve2d_fast(gray, kernel_sobel_x())),
        'Собель |∇|': sobel_magnitude(gray),
    }
...
def plot_threshold_comparison(image_rgb, edge_method='sobel'):
    """Сравнение разных порогов бинаризации."""
    gray = to_grayscale(image_rgb)
    blurred = convolve2d_fast(gray, kernel_gaussian_blur(5, 1.5))

    if edge_method == 'sobel':
        edges = normalize_image(sobel_magnitude(blurred))
    else:
...
def plot_edge_methods_comparison(image_rgb):
    ...
    methods = ['sobel', 'laplacian', 'laplacian_full', 'custom']
    method_names = ['Собель', 'Лапласиан (4)', 'Лапласиан (8)', 'С доски']
...
def main():
    ...
    for name, img in test_images.items():
        print(f"\n  --- {name} ---")
        results = detect_objects(img, blur_sigma=1.5, blur_size=5,
                                 edge_method='sobel', threshold=0.15,
                                 dilate_radius=2, min_area=30)
    ...
    fig_thr = plot_threshold_comparison(test_images['Простой (контрастный фон)'])
